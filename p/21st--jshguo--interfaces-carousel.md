<!-- Interfaces Carousel · @jshguo · https://21st.dev/@jshguo/components/interfaces-carousel
...
export default function CarouselVerticalDemo() {
  return (
    <div className="flex w-full min-h-screen items-center justify-center overflow-hidden bg-background p-8">
      <Carousel orientation="vertical" className="w-full max-w-xs sm:max-w-sm">
...
```bash
npm install @carbon/icons-react @radix-ui/react-slot class-variance-authority embla-carousel-react lucide-react
```

Install shadcn/ui registry dependencies:
```bash
npx shadcn@latest add button styles utils
```
