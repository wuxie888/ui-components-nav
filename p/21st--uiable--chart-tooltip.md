<!-- Chart Tooltip · @uiable · https://21st.dev/@uiable/components/chart-tooltip
     license: MIT · category: dashboard
     A styled tooltip content block for data charts that shows labeled values with colored dot, line, or dashed indicators next to each series. -->
...
```tsx
components/uiable/chart/chart-tooltip.tsx
...
export default function ChartTooltipDemoUsage() {
  return <ChartTooltip />;
}
```
