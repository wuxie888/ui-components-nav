<!-- Gate Shift · @zzzzshawn · https://21st.dev/@zzzzshawn/components/dotm-circular-7
     license: no-license · category: grid
     Animated circular dot-matrix loader whose scanning gate alternates between vertical and horizontal sweeps, producing a crisp shutter rhythm across a masked grid of dots. -->
...
```tsx
components/ui/dotm-circular-7.tsx
"use client";
...
export default function DotmCircular7Demo() {
  return (
    <div className="flex min-h-[280px] w-full items-center justify-center bg-background text-foreground">
      <DotmCircular7 size={150} dotSize={16} />
    </div>
  );
}
```
